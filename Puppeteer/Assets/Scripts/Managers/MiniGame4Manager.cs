...
public class MiniGame4Manager : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
    ...
    float accuracy       = 0f;
    float materialDamage = 0f;
    bool  confirmed      = false;

    int   levelsSucceeded = 0;
        ...
        foreach (var btn in canvas.GetComponentsInChildren<Button>(true))
        {
            switch (btn.gameObject.name)
            {
                case "SkipBtn":
                    ...
                    btn.onClick.AddListener(OnSkip);
                    break;
                case "UndoBtn":
                    btn.onClick.RemoveAllListeners();
                    btn.onClick.AddListener(OnUndo);
                    break;
                case "ConfirmBtn":
                    btn.onClick.RemoveAllListeners();
    ...
    public void OnNext()
    {
        ...
        if (topAccuracyText != null) topAccuracyText.text = "Painted: 0%";
    ...
    void BuildTopHUD()
    {
        ...
        topAccuracyText = MakeHUDLabel(canvas, "HUD_Accuracy",
            new Vector2(0, 1), new Vector2(0, 1), new Vector2(0, 1),
            new Vector2(32, -24), new Vector2(320, 60));
        topAccuracyText.alignment = TextAnchor.UpperLeft;
        topAccuracyText.text = "Painted: 0%";

        topTimerText = MakeHUDLabel(canvas, "HUD_Timer",
            new Vector2(1, 1), new Vector2(1, 1), new Vector2(1, 1),
            new Vector2(-32, -24), new Vector2(200, 60));
    ...
    void BuildMarcusText()
    {
        ...
        marcusText.text          = "Marcus: Paint the face carefully! Stay inside the outline…";
    ...
    void BuildOverlay()
    {
        ...
        // Start transparent — player paints the face
    ...
    void PaintCircle(int cx, int cy, int r)
    {
        ...
        for (int dy = -r; dy <= r; dy++)
        {
            ...
            overlayTex.SetPixel(x, y, paintColor);
        }
    }
    ...
    void RefreshAccuracy()
    {
        ...
        {
            // paintedBody    = painted AND inside body  (good coverage)
            // paintedOutside = painted AND outside body (damage, penalised)
            int paintedBody = 0, paintedOutside = 0;
            for (int i = 0; i < px.Length; i++)
            {
                bool painted = px[i].a > 10;
                if      (painted &&  shapeMask[i]) paintedBody++;
            ...
            materialDamage     = outsideRatio;
            accuracy = Mathf.Clamp01(coverage - penalty * outsideRatio) * 100f;

            if (paintedOutside > prevPaintedOutside)
                Debug.Log($"Painted: {accuracy:F1}% | Damage: {materialDamage:F2}");
            else
                Debug.Log($"Painted: {accuracy:F1}%");
            prevPaintedOutside = paintedOutside;
        ...
        {
            int painted = 0;
            foreach (Color32 c in px) if (c.a > 10) painted++;
            accuracy = (float)painted / px.Length * 100f;
            Debug.Log($"Painted: {accuracy:F1}%");
        }

        if (accuracyText)    accuracyText.text    = $"{accuracy:F0}%";
        if (accuracySlider)  accuracySlider.value  = accuracy / 100f;
        if (topAccuracyText) topAccuracyText.text  = $"Painted: {Mathf.FloorToInt(accuracy)}%";
    }
    ...
    public void OnConfirm()
    {
        ...
        if (!isLastLevel && nextButton    != null) nextButton.gameObject.SetActive(true);
        if ( isLastLevel && completeButton != null) completeButton.gameObject.SetActive(true);
    }
